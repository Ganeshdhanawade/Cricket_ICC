...
class DataPrep(BaseEstimator, TransformerMixin):
    ...
    def transform(self,df, y=None):
        ...
        df['Runs in Last 5 Overs'] = df['Runs Scored still that over'].diff(periods=5).fillna(0)
        df['Wickets in Last 5 Overs'] = df['Wickets Scored still that over'].diff(periods=5).fillna(0)
